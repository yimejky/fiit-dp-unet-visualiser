...
def block(conv_num, pool_num, in_channels, out_channels, height, height_out, to=None, width=2, width_scaller=2):
    ...
    return [
        to_Conv(f"conv{cn}", '', in_channels, offset="(1,0,0)", to=to, height=height, depth=height, width=width),
...
arch.extend([
    to_Conv("output", '', 1, offset="(1,0,0)", to=f"(unconv8-east)", height=max_height, depth=max_height, width=width),
